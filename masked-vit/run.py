...
def main():
    ...
    ddr = DDRSegmentationDataModule("data/DDR-dataset", **kwargs)
    maples = MaplesSegmentationDataModule("data/MAPLES-DR", **kwargs)
    aptos = AptosClassificationDataModule("data/APTOS2019", **kwargs)
...
def run(dm: LightningDataModule, dataset_name: str, compute_masks: bool, keep_ps: Sequence[float]) -> None:
    dm.setup("fit")
    ...
    for keep_p in keep_ps:
        metrics = compute_metrics(model, dataloader, DEVICE, compute_masks=compute_masks)
        plot_confusion_matrices(metrics, dataset_name)

        df = as_dataframe(metrics)
        print(f"\n{dataset_name} ({keep_p = })\n")
        print(df)
...
def as_dataframe(metrics: dict[str, MetricCollection]) -> pd.DataFrame:
