...
from typing import Any, Callable, Iterator, List, Literal, Tuple, TypedDict
...
class Aptos(InMemoryDataset):
    ...
    def processed_dir(self) -> str:
        if self.mode == "SIFT":
            return osp.join(self.root, f'processed_{self.mode}_{self.pre_transform_kwargs["num_keypoints"]}')
        elif self.mode == "LESIONS":
            return osp.join(
                self.root,
                f'processed_{self.mode}_{self.pre_transform_kwargs["which_features"]}_{self.pre_transform_kwargs["feature_layer"]}',
            )
        else:
...
def extract_features_by_cc(cc, features, nlabel):
    if nlabel == 1:
...
class _LesionsTransform:
    def __init__(
        self, which_features: Literal["decoder", "encoder"] = "encoder", feature_layer: int = 3, compile=False
    ):
    ...
    def __call__(self, img_path: Path, label: int) -> Data:
        ...
        cctorch = torch.from_numpy(cc.astype(np.int64)).unsqueeze(0).unsqueeze(0).to(self.device)
        features_points = self.extract_features_by_cc(cctorch, features, nlabel)
